"""
DeepL Translate Activities for Temporal.

Activities for entry translation using DeepL API.
"""
...
from buun_curator.services.deepl_translator import DeepLTranslator

logger = get_logger(__name__)


@activity.defn
async def deepl_translate_entries(
    input: TranslateEntriesInput,
) -> TranslateEntriesOutput:
    """
    Translate a list of entries using DeepL API.
    ...
    if not config.deepl_api_key:
        logger.error("DEEPL_API_KEY not configured")
        return TranslateEntriesOutput()

    logger.info(
        "DeepL translate start",
        entries=len(input.entries),
        target_language=input.target_language,
    )

    # Detect if using Pro API (Pro keys don't end with ":fx")
    use_pro = not config.deepl_api_key.endswith(":fx")
    ...
    activity.heartbeat(f"Starting DeepL translation: {total} entries")
