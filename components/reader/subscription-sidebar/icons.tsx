 ...
 *
 * @param subscription - The subscription to get icon for
 * @returns React node with the appropriate icon
 */
export function getSubscriptionIcon(
  subscription: Subscription,
): React.ReactNode {
  if (subscription.id === "starred") {
    return <Star className="size-4 text-muted-foreground" />;
  }
  if (subscription.id === "all") {
    ...
    return (
      <CircleSmall
        className={cn("size-4 text-muted-foreground", {
          "fill-current": subscription.count && subscription.count > 0,
        })}
      />
    );
  }
  return <Rss className="size-4" />;
}
